...
export interface LogoMarkProps {
  size?: number;
  className?: string;
  /** Adds a soft ambient glow (used on the loading screen). */
  glow?: boolean;
}
...
/**
 * The Axiony mark - a gradient hexagon with an "A" built from connected
 * graph nodes (a green apex node + two base nodes joined by edges). Reads
 * as "accessibility, integrated".
 */
export function LogoMark({ size = 28, className, glow }: LogoMarkProps) {
  return (
    <span
    ...
    >
      <svg viewBox="0 0 64 64" fill="none" aria-hidden="true">
        ...
        {/* Edges of the node graph, forming an "A" */}
...
export function LogoLockup({ className, markSize = 72, tagline = true }: LogoLockupProps) {
  return (
