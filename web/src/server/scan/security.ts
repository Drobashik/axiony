...
const ensurePublicHost = async (hostname: string) => {
  ...
  if (isIP(normalized)) {
    if (isPrivateAddress(normalized)) {
      throw new ScanRequestError("Internal or private IP addresses cannot be scanned.");
    }
    return;
  }
  ...
  try {
    ...
    throw new ScanRequestError("Could not resolve that domain. Check the URL and try again.");
  }
...
export const validateScanRequest = async (body: unknown): Promise<{ url: string; level: WcagLevel }> => {
  ...
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    throw new ScanRequestError("Only http:// and https:// URLs can be scanned.");
  }

  if (parsed.username || parsed.password) {
    throw new ScanRequestError("URLs with embedded credentials cannot be scanned.");
  }
