...
export const resolveReactDependencies = (componentPath: string): ReactDependencyPaths => {
  const componentRequire = createRequire(join(dirname(componentPath), 'package.json'));
  ...
  try {
    ...
    const dependencyError = new Error(MISSING_REACT_DEPENDENCIES_MESSAGE) as Error & {
      cause?: unknown;
    };
